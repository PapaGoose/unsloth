    ...
    def patch_peft_model(
        ...
        if lora_dropout == 0 and bias == "none":
            for idx, layer in enumerate(model.model.model.layers):
                ...
                # w1 = layer.block_sparse_moe.experts.w1
                # w2 = layer.block_sparse_moe.experts.w2
                # w3 = layer.block_sparse_moe.experts.w2

                # if  hasattr(gate, "lora_A") and \
                #     hasattr(w1, "lora_A") and \
                #     hasattr(w2, "lora_A") and \
                #     hasattr(w3, "lora_A") and \
                #     (gate.base_layer if hasattr(gate, "base_layer") else gate).bias is None and \
                #     (w1.base_layer if hasattr(gate, "base_layer") else w1).bias is None and \
                #     (w2.base_layer if hasattr(gate, "base_layer") else w2).bias is None and \
                #     (w3.base_layer if hasattr(gate, "base_layer") else w3).bias is None:

                #     # https://stackoverflow.com/questions/50599045/python-replacing-a-function-within-a-class-of-a-module
                #     layer.block_sparse_moe.forward = types.MethodType(apply_lora_mlp, layer.mlp)
                #     n_moe += 1
                # else:
                #     logger.warning_once(
                #         "Unsloth cannot patch MOE layers with our manual autograd engine since either LoRA adapters\n"\
